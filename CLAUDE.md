...
- [scripts/1-create-gpu-cluster.sh](scripts/1-create-gpu-cluster.sh), [scripts/2-setup-arc.sh](scripts/2-setup-arc.sh), and [scripts/3-teardown-cluster.sh](scripts/3-teardown-cluster.sh) create/tear down the `kind` + ARC cluster this CI setup depends on ([scripts/kubernetes-gpu-arc-referencia.sh](scripts/kubernetes-gpu-arc-referencia.sh) is a commented study reference, not meant to be run end to end). They resolve their own directory (`$BASH_SOURCE`), so they work from any cwd once cloned, and read `k8s/kind-gpu-config.yaml` / `k8s/gpu-runner-values*.yaml` directly instead of regenerating copies under `~/` — those checked-in files are the single source of truth. `2-setup-arc.sh REPO [python|docker]` templates whichever `k8s/gpu-runner-values*.yaml` is picked with the given `REPO`'s `githubConfigUrl` (default variant is `docker`, since `python` needs `k8s/runner-image/Dockerfile` built and pushed first — see below). [.vscode/tasks.json](.vscode/tasks.json) wraps the first three as VS Code tasks ("K8s: criar cluster GPU", "K8s: configurar ARC", "K8s: recriar cluster do zero", "K8s: derrubar cluster") — `GITHUB_TOKEN` is only prompted for by the tasks that actually need it (create/recreate), via a `promptString` input, not hardcoded anywhere.
